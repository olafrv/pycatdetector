    ...
    def record(self, greyscale=False, showVisor=False, saveImage=False):
        while(not self.must_stop):
            ...
            if not cap.isOpened():
                ...
                self.logger.info("Connected. FPS: " + str(fps) + ", SKIP: " + str(frames_to_skip))
                ...
                while(not self.must_stop):
                    ...
                    if corrupted: 
                        ...
                        if corrupted_count > self.CORRUPTED_MAX_FRAMES:
                            self.logger.error(
                                "Reached max amount of corrupted frames. Shape: " 
                                + str(frame.shape if frame is not None else "-")
                            )                              
                            break    # retry conn
                        else:
                            self.logger.debug("[Corrupted] In:" + str(total_reads) + ", Out: " + str(total_writes) + ", Queue: " + str(self.images.qsize()))
                            continue # skip frame
                    else:
                        ...
                        if reads % frames_to_skip == 0:
                            ...
                            self.logger.debug("In:" + str(total_reads) + ", Out: " + str(total_writes) + ", Queue: " + str(self.images.qsize()))
            ...
            if conn_error: 
                self.logger.info("Retrying in " + str(self.RECONNECT_DELAY) + " seconds ...")
