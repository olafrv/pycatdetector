    ...
    def notify(self, custom_content: Optional[dict] = None) -> bool:
        ...
        try:
            ...
            if custom_content is not None:
                self.logger.debug(
                    f"Custom content provided but not used: {custom_content}"
                )
            ...
            if response.status_code == 200:
                self.logger.info(
                    f"BlinkstickSquare notification sent successfully to {self.url}"
                )
                return True
            else:
                self.logger.error(
                    f"BlinkstickSquare notification failed: HTTP {response.status_code} - {response.text}"
                )
                return False

        except requests.exceptions.RequestException as e:
            self.logger.error(
                f"BlinkstickSquare notification failed with exception: {e}"
            )
            return False
